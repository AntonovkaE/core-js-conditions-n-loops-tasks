 ...
 *  26  => XXVI
 */
function convertToRomanNumerals(numb) {
  let num = numb;
  let result = '';
  const rom = {
    X: 10,
  ...
  if (num / rom.X >= 1) {
    for (let i = 1; i <= num / rom.X; i += 1) {
      result += 'X';
    }
    num %= rom.X;
  }
  if (num / rom.IX >= 1) {
    for (let i = 1; i <= num / rom.IX; i += 1) {
      result += 'IX';
    }
    num %= rom.IX;
  }
  if (num / rom.V >= 1) {
    for (let i = 1; i <= num / rom.V; i += 1) {
      result += 'V';
    }
    num %= rom.V;
  }
  if (num / rom.IV >= 1) {
    for (let i = 1; i <= num / rom.IV; i += 1) {
      result += 'IV';
    }
    num %= rom.IV;
  }
  if (num / rom.I >= 1) {
    for (let i = 1; i <= num / rom.I; i += 1) {
      result += 'I';
    }
  }
  return result;
...
function getSpiralMatrix(size) {
  ...
  while (startRow <= endRow && startColumn <= endColumn) {
    ...
    startRow += 1;
    for (let k = startRow; k <= endRow; k += 1) {
      arr[k][endColumn] = count;
      count += 1;
    }
    endColumn -= 1;
    if (startRow <= endRow) {
      for (let j = endColumn; j >= startColumn; j -= 1) {
        arr[endRow][j] = count;
        count += 1;
      }
      endRow -= 1;
